        ...
        $(` <div id="despesa${i} " class="conteudoIndividual"> 
                <div class="despesas linha" > 
                   ...
                   <input type="text" name="">
                    <input type="text" name="quantidade"  class="quantidade total" onblur="teste()"/>
                    <input type="text" name="valor_unitario"  class="valor_unitario total" onblur="teste()"/>
   ...
   const totalGeral = []
    ...
    $('.linha').each(function (){
        ...
        if(isNaN(total)){
            ...
            $(this).find('input[name=valor_total]').val(total.toLocaleString("pt-BR", { style: "currency" , currency:"BRL"})).html(total)
        }
    ...
    $('.linha').each(function (){
        ...
        var total1 =  $(this).find('input[name=valor_total]').val().replace(/[^0-9,]*/g, '').replace(',', '.')
...
function valorTotal (){
    ...
    var teste = parseFloat( $(this).find('input[name=valor_total]').val().replace(/[^0-9,]*/g, '').replace(',', '.'))

    if (typeof teste == "number"  && !isNaN(teste)){
        
        s1 += teste
         $('.totalDespesas > p').html(s1.toLocaleString("pt-BR", { style: "currency" , currency:"BRL"}));
    }else{
